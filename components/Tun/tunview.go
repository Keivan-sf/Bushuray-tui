...
func (m Model) View() string {
	...
	sudo_bushuray := lipgloss.NewStyle().
		...
		Render("sudo bushuray")

	dialog_text := lipgloss.
		JoinHorizontal(
			lipgloss.Left,
			bg_style.Render("Core is not running as root, do you want to kill it so you can start again with "),
			sudo_bushuray,
			bg_style.Render(" ?"))
